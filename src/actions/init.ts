...
async function branchOnboardingFlow(context: TContext) {
  context.splog.tip(
    [
      ...
      'To add other non-Graphite branches to Graphite later, check out `gt track`.',
      'If you only want to use Graphite for new branches, feel free to exit now and use `gt create`.',
    ].join('\n')
  );
  if (
    !(
